...
# ============================================================
# PAYLOAD اللي استخرجته من جهازك النظيف (BASE64 الطويل)
# ============================================================
...
def send_login_request(username: str, password: str):
    ...
    # الإرسال إلى سناب
    r = requests.post(SNAP_URL, headers=HEADERS, data=grpc_body, timeout=15)
...
def login():
    ...
    status, response = send_login_request(data['username'], data['password'])
    return {'status': status, 'response': response[:500]}

@app.route('/')
def index():
    return {'message': 'Snapchat Bypass API is running', 'endpoint': '/login'}
