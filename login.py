import requests
import base64
import struct
import blackboxprotobuf
from flask import Flask, request, jsonify

app = Flask(__name__)

# ============================================================
# ضع هنا PAYLOAD_B64 كاملاً (الـ base64 الطويل جداً)
# الذي استخرجته من جهازك النظيف
# ============================================================
PAYLOAD_B64 = ""

# ============================================================
# الهيدرز الثابتة (من نفس الجهاز النظيف)
# ============================================================
HEADERS_FIXED = {
    "Content-Type": "application/grpc+proto",
    "User-Agent": "Snapchat/12.85.0.44 (iPhone9,3; iOS 15.7.5; gzip)",
    "X-Snapchat-UUID": "93F3D290-1729-4593-B346-F297C7E2F677",
    "x-snapchat-att-token": "Ci1EfdA0txvTV2_uCAnIfHe9UU4HEmr92o1p1fU40FW8inWat0leOlO-Y1ZYuckVAQAAAA==",
    "te": "trailers",
}

SNAP_LOGIN_URL = "https://us-east1-aws.api.snapchat.com/snapchat.janus.api.LoginService/LoginWithPassword"

def snap_login(username, password):
    try:
        proto_bytes = base64.b64decode(PAYLOAD_B64)
        decoded, typedef = blackboxprotobuf.decode_message(proto_bytes)
        decoded['1'] = username.encode('utf-8')
        decoded['4'] = password.encode('utf-8')
        new_bytes = blackboxprotobuf.encode_message(decoded, typedef)
        grpc_body = struct.pack('>BI', 0, len(new_bytes)) + new_bytes
        r = requests.post(SNAP_LOGIN_URL, headers=HEADERS_FIXED, data=grpc_body, timeout=20)
        return r.status_code, r.text[:500]
    except Exception as e:
        return 500, str(e)

@app.route('/login', methods=['POST'])
def handle_login():
    data = request.get_json()
    if not data or 'username' not in data or 'password' not in data:
        return jsonify({"error": "Please send username and password in JSON"}), 400
    status, resp_text = snap_login(data['username'], data['password'])
    return jsonify({"status": status, "response_preview": resp_text}), 200

@app.route('/', methods=['GET'])
def home():
    return jsonify({"message": "Snapchat Bypass API is running", "method": "POST", "endpoint": "/login", "example": '{"username": "user", "password": "pass"}'})

if __name__ == "__main__":
    app.run(host="0.0.0.0", port=10000)
