...
# ============================================================
# بايلود جهازك النظيف (BASE64 الطويل حقتك أنت)
# ============================================================
...
def send_request(username, password):
    # 1. فك BASE64 (حقتك أنت)
    full = base64.b64decode(PAYLOAD_B64)
    
    # 2. بما أن البايلود حقتك فيها gRPC frame, نشيل أول 5 بايتات عشان نصير protobuf خالص
    if len(full) > 5 and full[0] == 0:
        proto = full[5:]
    else:
        proto = full
    
    # 3. نستخدم blackboxprotobuf عشان نعدل
    import blackboxprotobuf
    decoded, typedef = blackboxprotobuf.decode_message(proto)
    decoded['1'] = username.encode()
    decoded['4'] = password.encode()
    
    # 4. نعيد الترميز ونضيف gRPC frame
...
def login():
    try:
        ...
        status, resp = send_request(username, password)
